...
class UncertaintyEstimator:
    """
    Bare-bones uncertainty estimator:
    ...
    def fit(self, df_with_preds: pd.DataFrame):
        ...
        # avoid sigma=0 in tiny datasets
    ...
    def attach_uncertainty(self, df_with_preds: pd.DataFrame) -> pd.DataFrame:
        ...
        # Normal approx quantiles (z for 10% and 90%)
    ...
    def sample_scenarios(
        ...
        """
        Generates demand scenarios per row using Normal(mu, sigma).
...
if __name__ == "__main__":
    # Example assumes you already produced predictions somewhere
    # e.g., via demand_model.py and saved/kept df with predicted_demand.
    df = pd.read_csv("data/demand_history.csv")

    # Fake baseline prediction just for standalone testing:
    # (In reality, you will use your DemandModel predictions.)
    ...
    print(test_with_u[["day", "store_id", "predicted_demand", "demand_sigma", "p10", "p50", "p90"]].head())

    scen = ue.sample_scenarios(test_df.head(3), n_scenarios=5)
    print("\nScenario sample:")
    print(scen[["scenario_id", "day", "store_id", "predicted_demand", "demand_sigma", "scenario_demand"]].head(10))
